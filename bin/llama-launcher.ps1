...
Write-Host "  4. 创建/刷新手动路由配置（router-models.ini）"
Write-Host ""
...
if ($mode -eq "4") {
    ...
    Write-ManualRouterPreset -ModelFiles $routerModels -MmprojFiles $mmprojFiles -OutputPath $manualRouterPreset
    ...
    Write-Host "如需多模态模型，请编辑对应模型段落，取消注释 mmproj 和 image-min-tokens。" -ForegroundColor Cyan
    Read-Host "按 Enter 退出"
    exit 0
}
