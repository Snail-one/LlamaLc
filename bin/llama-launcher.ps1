...
function Write-ManualRouterPreset {
    ...
    $lines = @()
    $lines += "; Generated by llama-launcher.ps1"
    $lines += "; Manual router preset. Edit this file as needed; launcher will not overwrite it unless you choose menu option 2."
    $lines += "; API model ids use GGUF filenames."
...
Write-Host "  0. 退出"
Write-Host ""
$mode = [string](Read-MenuChoice -Prompt "请选择运行方式" -Default 1 -Min 0 -Max 4)
if ($mode -eq "0") {
    break MainMenu
}
...
$modelIndex = Read-MenuChoice -Prompt "请选择模型编号" -Default $recommendedModelIndex -Min 0 -Max $models.Count
if ($modelIndex -eq 0) {
    continue MainMenu
}
